...
def generate_dockerfile(base_image_name: str) -> None:
    ...
    missing = []
    if not hasattr(ConfigurationPlotclip, 'transformer_name') or not ConfigurationPlotclip.transformer_name:
        missing.append("Transformer name")
    if not hasattr(ConfigurationPlotclip, 'author_name') or not ConfigurationPlotclip.author_name:
        missing.append("Author name")
    if not hasattr(ConfigurationPlotclip, 'author_email') or not ConfigurationPlotclip.author_email:
        ...
        with open(dockerfile_name, 'w') as out_file:
            for line in template:
                if line.startswith('LABEL maintainer='):
                    out_file.write("LABEL maintainer=\"{0} <{1}>\"\n".format(ConfigurationPlotclip.author_name, \
                                   ConfigurationPlotclip.author_email))
